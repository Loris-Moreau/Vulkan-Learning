...
int GJK::CompareSigns(float a, float b)
{
    if (a > 0.0f && b > 0.0f) {
        return 1;
    }
...
Vec2 GJK::SignedVolume1D(const Vec3& s1, const Vec3& s2)
{
    // Ray from 'a' to 'b'
    Vec3 ab = s2 - s1;
...
Vec3 GJK::SignedVolume2D(const Vec3& s1, const Vec3& s2, const Vec3& s3)
{
...
Vec4 GJK::SignedVolume3D(const Vec3& s1, const Vec3& s2, const Vec3& s3, const Vec3& s4)
{
    Mat4 M;
...
void GJK::TestSignedVolumeProjection()
{
	const Vec3 orgPts[4] = {
		Vec3(0, 0, 0),
		Vec3(1, 0, 0),
...
bool GJK::SimplexSignedVolumes(Point* pts, const int num, Vec3& newDir, Vec4& lambdasOut)
{
...
bool GJK::HasPoint(const Point simplexPoints[4], const Point& newPt)
{
...
void GJK::SortValids(Point simplexPoints[4], Vec4& lambdas)
{
	bool valids[4];
	for (int i = 0; i < 4; i++) {
		valids[i] = true;
...
int GJK::NumValids(const Vec4& lambdas)
{
	int num = 0;
	for (int i = 0; i < 4; i++) {
		if (0.0f != lambdas[i]) {
...
bool GJK::GJK_DoesIntersect(const Body* bodyA, const Body* bodyB)
{
...
Vec3 GJK::BarycentricCoordinates(Vec3 s1, Vec3 s2, Vec3 s3, const Vec3& pt) {
	s1 = s1 - pt;
	s2 = s2 - pt;
	s3 = s3 - pt;
...
Vec3 GJK::NormalDirection(const Tri& tri, const std::vector< Point >& points) {
...
float GJK::SignedDistanceToTriangle(const Tri& tri, const Vec3& pt, const std::vector<Point>& points)
{
...
int GJK::ClosestTriangle(const std::vector<Tri>& triangles, const std::vector<Point>& points)
{
...
bool GJK::HasPoint(const Vec3& w, const std::vector< Tri > triangles, const std::vector< Point >& points) {
...
int GJK::RemoveTrianglesFacingPoint(const Vec3& pt, std::vector<Tri>& triangles, const std::vector<Point>& points)
{
	int numRemoved = 0;
	for (int i = 0; i < triangles.size(); i++) {
		const Tri& tri = triangles[i];
...
void GJK::FindDanglingEdges(std::vector<Edge>& danglingEdges, const std::vector<Tri>& triangles)
{
	danglingEdges.clear();
...
float GJK::EPA_Expand(const Body* bodyA, const Body* bodyB, const float bias, const Point simplexPoints[4], Vec3& ptOnA, Vec3& ptOnB) {
...
bool GJK::GJK_DoesIntersect(const Body* bodyA, const Body* bodyB, const float bias, Vec3& ptOnA, Vec3& ptOnB)
{
	const Vec3 origin(0.0f);
...
void GJK::GJK_ClosestPoints(const Body* bodyA, const Body* bodyB, Vec3& ptOnA, Vec3& ptOnB)
{
	const Vec3 origin(0.0f);
